...
function App() {
  ...
  useEffect(() => {
    const fetchWeather = async () => {
      if (navigator.geolocation) {
        navigator.geolocation.getCurrentPosition(async (position) => {
          ...
          await fetch(
            `${weatherAPI.base}weather?lat=${pos.lat}&lon=${pos.lon}&units=metric&appid=${weatherAPI.key}`
          )
            .then((res) => {
              if (res.ok) return res.json();
    ...
    const fetchNews = async () => {
      if (news.articles.length === 1) {
        await fetch(
          `${newsAPI.base}top-headlines?&lang=hi&token=${newsAPI.key}`
        )
  ...
  return (
    <div className="App">
      ...
      <div className="lan">
        <button onClick={() => changeLan("Hindi")} data-testid="lang-hi">
          हिन्दी
        </button>
        <button onClick={() => changeLan("English")} data-testid="lang-en">
          English
        </button>
      </div>
